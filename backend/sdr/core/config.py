...
class Settings(BaseSettings):
    ...
    AI_BATCH_DEBATE_UNGROUNDED_NOT_MET_POLICY: str = Field(default="downgrade_na")
    AI_BATCH_DEBATE_CHILD_REFINE_ENABLED: bool = Field(default=True)
    AI_BATCH_DEBATE_CHILD_REFINE_MAX_CONTEXT_CHUNKS: int = Field(default=6)
    AI_BATCH_DEBATE_CHILD_REFINE_INCLUDE_SOURCE_BLOCKS: bool = Field(default=True)
    AI_BATCH_DEBATE_CHILD_REFINE_SOURCE_BLOCK_LIMIT: int = Field(default=8)
    AI_BATCH_DEBATE_CHILD_REFINE_ENABLE_KEYWORD_BOOST: bool = Field(default=True)
    AI_BATCH_DEBATE_CHILD_REFINE_ENABLE_RERANK: bool = Field(default=True)
    AI_DEBATE_CONTEXT_SUPPLEMENTAL_BLOCK_LIMIT: int = Field(default=12)
    AI_DEBATE_SUPPLEMENTAL_BLOCK_WINDOW_BEFORE: int = Field(default=1)
    AI_DEBATE_SUPPLEMENTAL_BLOCK_WINDOW_AFTER: int = Field(default=1)
    AI_DEBATE_SUPPLEMENTAL_BLOCK_CHAR_BUDGET: int = Field(default=2200)
    ...
    AI_PARENT_RETRIEVAL_MAX_CONTEXT_CHUNKS: int = Field(default=6)
    ...
    AI_RETRIEVAL_GRAPH_LOCAL_MAX_WORKERS: int = Field(default=3)
    AI_RETRIEVAL_MANY_MAX_CONCURRENCY: int = Field(default=2)
    AI_RETRIEVAL_ENABLE_CROSS_ENCODER_RERANK: bool = Field(default=False)
    ...
    AI_TSD_PREPARATION_ENABLED: bool = Field(default=True)
    AI_TSD_PREPARATION_AUTO_START_ON_UPLOAD: bool = Field(default=True)
    AI_TSD_PREPARATION_SCHEMA_VERSION: int = Field(default=1)
    AI_TSD_PREPARATION_CACHE_ENABLED: bool = Field(default=True)
    AI_TSD_PREPARATION_CACHE_MAX_ITEMS: int = Field(default=8)
    AI_TSD_PREPARATION_CACHE_TTL_SECONDS: int = Field(default=1800)
    AI_TSD_PREPARATION_ARTIFACT_COMPRESSION: str = Field(default="zstd")
    AI_TSD_PREPARATION_VECTOR_BACKEND: str = Field(default="pgvector")
