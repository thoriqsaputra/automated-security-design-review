...
from sdr.apps.ai.prompts.retrieval import (
...
class GraphCommunityService:
    ...
    def summarize_communities(self, graph: TSDGraph, communities: List[GraphCommunity]) -> List[CommunitySummary]:
        ...
        for community in communities:
            ...
            if self.enable_llm_summary:
                llm_summary = self._llm_summary(community, fallback)
                if llm_summary:
    ...
    def _llm_summary(self, community: GraphCommunity, fallback: Dict[str, Any]) -> Optional[Dict[str, str]]:
        try:
            prompt = build_graph_community_summary_prompt(
                key_entities=fallback["key_entities"],
                key_relationships=fallback["key_relationships"],
                block_ids=community.block_ids[:10],
            )
            response = chat_completion(
                messages=[
                    {"role": "system", "content": GRAPH_COMMUNITY_SUMMARY_SYSTEM_PROMPT},
...
__all__ = ["CommunitySummary", "GraphCommunity", "GraphCommunityService"]
