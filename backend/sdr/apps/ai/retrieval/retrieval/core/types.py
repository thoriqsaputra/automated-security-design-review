...
class AdvancedRetrievalConfig:
    ...
    # dense floor = raptor_low's top-k, so hybrid coverage ⊇ raptor_low's.
    protected_dense_top_n: int = 7
    protected_bm25_top_n: int = 3
    protected_raptor_top_n: int = 3
    ...
    def from_settings(cls) -> "AdvancedRetrievalConfig":
        return cls(
            ...
            protected_raptor_top_n=max(0, int(getattr(settings, "AI_RETRIEVAL_PROTECTED_RAPTOR_TOP_N", 2))),
        )
