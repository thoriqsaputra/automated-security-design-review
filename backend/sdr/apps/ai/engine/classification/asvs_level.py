...
def classify_tsd_asvs_level(tsd_text: str, levels: Iterable[Any]) -> ASVSLevelClassification:
    ...
    prompt = build_asvs_level_classification_prompt(
    ...
    try:
        response = chat_completion(
            messages=[
                {
                    "role": "system",
                    "content": ASVS_LEVEL_CLASSIFICATION_SYSTEM_PROMPT,
                },
        ...
        if not isinstance(parsed, dict):
            return ASVSLevelClassification(
                level=1,
                confidence=0.0,
                reasoning="ASVS level classification could not be parsed; defaulted to L1.",
                evidence=[],
