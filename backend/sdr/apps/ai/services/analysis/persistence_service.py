...
from .dto import PersistenceInput, AnalysisSummary
from .severity import calculate_deterministic_severity
...
class PersistenceService:
    ...
    def persist_finding(
        ...
        try:
            ...
            for diagram_input, vision_result in debate_output.vision_results:
                self._persist_diagram_finding(
                    review=review,
                    parameter=parameter,
    ...
    def _persist_diagram_finding(
        ...
        try:
            if getattr(vision_result, "error", None):
                self.logger.warning(
                    "PersistenceService._persist_diagram_finding: skipping finding creation due to error for diagram_id=%s",
                    diagram_input.diagram_id,
                )
                return

            vision_details = getattr(vision_result, "details", {}) or {}
            met_status = getattr(vision_result, "verdict", None)
            severity_payload = self._build_diagram_severity_payload(
                met_status=met_status,
                confidence_score=getattr(vision_result, "confidence", None),
                domain=(
                    parameter.parent.category.code
                    if parameter.parent and parameter.parent.category
                    else None
                ),
            ...
            status_val = FindingStatus.OPEN.value if hasattr(FindingStatus, 'value') else FindingStatus.OPEN

            with SessionLocal() as db:
                qs = select(Finding).where(
                    Finding.review_id == review.id,
                    Finding.child_parameter_id == parameter.id,
                ...
                if not finding:
                    finding = Finding(
                        ...
                        title=self._strip_null_bytes(diagram_display["title"]),
                        description=self._strip_null_bytes(vision_result.reasoning or ""),
                        status=status_val,
                        met_status=met_status,
                        confidence_score=getattr(vision_result, "confidence", None),
                        severity=severity_payload["severity"],
                        severity_score=severity_payload["severity_score"],
                        severity_analysis=severity_payload["severity_analysis"],
                        reason=self._strip_null_bytes(vision_result.reasoning),
                        diagram_caption=self._strip_null_bytes(diagram_display["caption"]),
                        vision_reasoning=self._strip_null_bytes(vision_result.reasoning),
                        vision_thought_process=self._strip_null_bytes(getattr(vision_result, "cot_trace", None)),
                        requirement_reference=parameter.stable_key,
                        requirement_text=self._strip_null_bytes(build_parameter_analysis_text(parameter)),
                        requirement_metadata=self._strip_null_bytes({
                            "source": "vision_agent",
                            ...
                            "vision_result": {
                                "visual_elements_cited": list(getattr(vision_result, "visual_elements_cited", []) or []),
                                "missing_controls": list(getattr(vision_result, "missing_controls", []) or []),
                                "ambiguous_elements": list(getattr(vision_result, "ambiguous_elements", []) or []),
                                "missing_information": list(getattr(vision_result, "missing_information", []) or []),
                            },
                        }),
            ...
            self.logger.info(
                "PersistenceService._persist_diagram_finding: [SUCCESS] created=%s finding_id=%s",
                created,
            ...
            self.logger.exception(
                "PersistenceService._persist_diagram_finding: failed: %s",
                exc,
            )
    ...
    def _build_diagram_display_payload(self, diagram_input, vision_result) -> dict:
        ...
        if clean_caption:
            ...
            architect_summary = getattr(vision_result, "architect_summary", {}) or {}
