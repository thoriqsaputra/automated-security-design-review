...
class CategoryParameterEmbedding(Base, StandardsBigIntBase):
    __tablename__ = "standards_categoryparameterembedding"

    TYPE_PARENT = "parent"
    TYPE_CHILD = "child"

    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    parameter_type: Mapped[str] = mapped_column(String(16))
    
    parent_id: Mapped[Optional[int]] = mapped_column(ForeignKey("standards_categoryparameterparent.id", ondelete="CASCADE"), nullable=True, index=True)
    child_id: Mapped[Optional[int]] = mapped_column(ForeignKey("standards_categoryparameterchild.id", ondelete="CASCADE"), nullable=True, index=True)
    ...
    __table_args__ = (
        ...
        Index("idx_categoryparameterembedding_content_hash", "content_hash"),
        Index(
            "embedding_hnsw_idx",
            "embedding",
