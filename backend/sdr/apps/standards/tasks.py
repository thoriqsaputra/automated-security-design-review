...
def run_standard_ingestion_job_sync(job_id: str, celery_task_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    with SessionLocal() as db:
        ...
        logger.info("run_standard_ingestion_job_sync: [STEP 1] loaded source documents. count=%d", len(source_docs))
        ...
        if summary['errors'] > 0:
            job.error_message = 'Document failed during ingestion.'
            job.is_active = False
        else:
            db.execute(
