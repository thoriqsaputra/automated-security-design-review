...
def test_build_retrieval_snapshot_serializes_raptor_only():
    pipeline = _pipeline()
    ...
    snapshot = pipeline._build_retrieval_snapshot(SimpleNamespace(raptor_tree=raptor_tree))
...
def test_run_debate_executes_escalation_round_on_low_confidence_overturn(monkeypatch):
    ...
    assert debate_output.mediator_result.final_verdict == "not_met"
    assert "verdict_policy" not in debate_output.analysis_trace
